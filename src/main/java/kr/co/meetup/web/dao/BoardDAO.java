...
public class BoardDAO {
	...
	// 모임 별 게시판 총 개수
	public int selectAllBoardCnt(int crewNo) {
		SqlSession ss = factory.openSession(true);
		int count = ss.selectOne("kr.co.meetup.web.board.selectAllBoardCnt",crewNo);
		ss.close();
		return count;
	}
	...
	// 게시판 카테고리별 게시글 조회
	public List<BoardVO> selectBoardByCategory(int crewNo, int boardCategoryNo,int startNo,int recordPerPage) {
		SqlSession ss = factory.openSession(true);
		HashMap<String, Integer> map = new HashMap<String, Integer>();
		map.put("crewNo", crewNo);
		map.put("boardCategoryNo", boardCategoryNo);
	...
	//리스트 페이징 처리용
	public List<BoardVO> selectBoardAll(int crewNo, int startNo,int recordPerPage) {
		SqlSession ss = factory.openSession(true);
		HashMap<String, Integer> map = new HashMap<String, Integer>();
		map.put("crewNo", crewNo);
		map.put("startNo", startNo);
